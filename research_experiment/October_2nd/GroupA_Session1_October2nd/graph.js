function defineGraph(){// create transition structures and define pictures set
    ...
    G.pairsMat = calPairsMat(G.transMat)
...
function calPairsMat(transMat) {
    var pairsMat=[[]];
	var iNode
	var jNode
	var nPairs=0;
	var pairsCounter=-1;
	for  (iNode=0;iNode<G.nNodes;iNode++){
        nPairs = nPairs + transMat[iNode].length
	}
	
	for  (iNode=0;iNode<G.nNodes;iNode++){
	    for  (jNode=0;jNode<transMat[iNode].length;jNode++){
	        pairsCounter = pairsCounter+1; 
	        pairsMat[pairsCounter].push( new Array(2));
	        pairsMat[pairsCounter][0] = iNode;
	        pairsMat[pairsCounter][1] = transMat[iNode][jNode];
	        if (pairsCounter<nPairs-1) { // add placeholder for next node.
			    pairsMat.push([]);
		    }	       
	    }
